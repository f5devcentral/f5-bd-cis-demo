...
# USER_TOKEN_VALUE=$(oc create token bigip-ctlr -n cis-mc-onetier --duration=0s)
USER_TOKEN_VALUE=$(oc -n cis-mc-onetier get secret bigip-ctlr -o=jsonpath='{.data.token}' | base64 -d )
